...
from extract_activation import (
    extract_mha_activation,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Compute and save standard deviation along the learned direction."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Type of activation to extract and trained on ('mha', 'mlp', or 'residual').",
    )
    parser.add_argument(
        "--concept",
        type=str,
    ...
    if concept == "assertiveness_calibration" and activation_type != "mha":
    ...
    if concept == "assertiveness_calibration":
        ...
        if not args.scored_csv:
            raise ValueError("--scored_csv required")
        chats, _ = construct_assertiveness_calibration_data(args.scored_csv)
    ...
    for datum in tqdm(
        tokenized_data, total=len(tokenized_data), desc="Extracting Activations"
    ):
        ...
        if concept == "assertiveness":
            ...
            if not os.path.exists(weights_path):
                raise FileNotFoundError(
                    f"Run train_epint first. Missing: {weights_path}"
                )
            ...
            for layer in tqdm(range(NUM_LAYER)):
                for head in range(NUM_HEAD):
                    ...
                    save_path = os.path.join(
                        probe_dir, f"linear_std_mha_{layer}_{head}.pt"
                    )
            ...
            for layer in tqdm(range(NUM_LAYER)):
                for head in range(NUM_HEAD):
                    if args.probe_type == "nonlinear":
                        direction = (
                            torch.load(
                                f"{probe_dir}/{args.probe_type}_probe_{layer}_{head}.pth"
                            )["net.2.weight"][0, :]
                            .cpu()
                            .to(torch.bfloat16)
                        )
                    ...
                    activations = torch.tensor(
                        tuning_activations[:, layer, head, :], dtype=torch.bfloat16
                    ).to("cpu")
                    proj_vals = activations @ direction.T
                    proj_val_std = torch.std(proj_vals)
                    torch.save(
                        proj_val_std,
                        f"{probe_dir}/{args.probe_type}_std_mha_{layer}_{head}.pt",
                    )
    elif activation_type == "residual":
        for layer in tqdm(range(NUM_LAYER)):
            if args.probe_type == "nonlinear":
                direction = (
                    torch.load(
                        f"trained_probe_{concept}/{model_id}/{args.probe_type}_probe_residual_{layer}.pth"
                    )["net.2.weight"][0, :]
                    .cpu()
                    .to(torch.bfloat16)
                )
            ...
            direction = direction / torch.norm(direction)
            activations = torch.tensor(
                tuning_activations[:, layer, :], dtype=torch.bfloat16
            ).to("cpu")
            proj_vals = activations @ direction.T
            proj_val_std = torch.std(proj_vals)
            torch.save(
                proj_val_std,
                f"trained_probe_{concept}/{model_id}/{args.probe_type}_std_residual_{layer}.pt",
            )
    elif activation_type == "mlp":
        for layer in tqdm(range(NUM_LAYER)):
            if args.probe_type == "nonlinear":
                direction = (
                    torch.load(
                        f"trained_probe_{concept}/{model_id}/{args.probe_type}_probe_mlp_{layer}.pth"
                    )["net.2.weight"][0, :]
                    .cpu()
                    .to(torch.bfloat16)
                )
            ...
            torch.save(
                proj_val_std,
                f"trained_probe_{concept}/{model_id}/{args.probe_type}_std_mlp_{layer}.pt",
            )
    else:
