...
def train_probe(
    ...
    """
    print(
        f"----------- Training {activation_type.upper()} Probe for Component: {target_component} --------------"
    )
    ...
    def collate_fn(batch):
        ...
        if is_gemma:
            ...
            pad_val = (
                ...
                if processor.pad_token_id is not None
                else processor.eos_token_id
            )
        ...
        for batch in train_loader:
            ...
            if activation_type == "mha":
                target_layer, target_head = [
                    int(x) for x in target_component.split("_")
                ]
                representations = inputs[:, target_layer, target_head, :].to(
                    torch.float32
                )
    ...
    if activation_type == "mha":
        ...
        save_path = os.path.join(
            save_dir, f"{probe_type}_probe_mlp_{target_component}.pth"
        )
    elif activation_type == "residual":  # mlp
        save_path = os.path.join(
            save_dir, f"{probe_type}_probe_residual_{target_component}.pth"
        )
...
def main(args):
    ...
    if is_gemma:
        ...
        if HEAD_DIM is None and args.activation_type == "mha":
            raise ValueError(
                "Cannot determine HEAD_DIM for MHA on this model architecture."
            )

    print(
        f"Model Config: Layers={NUM_LAYERS}, Heads={NUM_HEADS}, HiddenDim={HIDDEN_DIM}, HeadDim={HEAD_DIM}"
    )
    ...
    chats_templated = processor.apply_chat_template(
        chats, add_generation_prompt=False, tokenize=False
    )
    tokenized_data = [
    ...
    train_tok, val_tok, train_labels, val_labels = (
        train_test_split(  # split again since we need validation for probe training
    ...
    if args.activation_type == "mha":
        ...
        raise ValueError(
            f"Unsupported activation_type: {args.activation_type}. Choose 'mha', 'mlp', or 'residual'."
        )

    train_activation_list = []
    for datum in tqdm(
        train_tok, total=len(train_tok), desc="Extracting Train Activations"
    ):
    ...
    if args.wandb:
        wandb.log(
            {
                "accuracies": wandb.Table(
                    columns=["component", "val_acc"],
                    data=[[k, v] for k, v in accuracies.items()],
                )
            }
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_id", type=str, required=True, help="'gemma-3' or 'llama-3.2'"
    )
    parser.add_argument(
        "--activation_type",
        type=str,
        required=True,
        choices=["mha", "mlp", "residual"],
        help="Type of activation to extract and train on ('mha', 'mlp', or 'residual').",
    )
    parser.add_argument(
        "--batch_size", type=int, default=25, help="Batch size for probe training."
    )
    parser.add_argument(
        "--lr", type=float, default=0.001, help="Learning rate for Adam optimizer."
    )
    parser.add_argument(
        "--epochs", type=int, default=25, help="Number of epochs for probe training."
    )
    parser.add_argument(
        "--concept",
        type=str,
