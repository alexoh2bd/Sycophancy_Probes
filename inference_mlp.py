...
def get_probe_vectors(
    ...
    for layer in target_layers:
        current_probe = torch.load(
            f"linear_probe/trained_probe_sycophancy/{model_id}/linear_probe_mlp_{layer}.pth"
        ...
        current_std = torch.load(
            f"linear_probe/trained_probe_sycophancy/{model_id}/std_mlp_{layer}.pt"
        )
...
def main():
    ...
    parser.add_argument(
        "--chosen_layer", type=int, default=1, help="Layer to intervene"
    )
    parser.add_argument(
    ...
    accuracies = pickle.load(
        open(
            f"linear_probe/trained_probe_sycophancy/{model_id}/accuracies_dict_mlp.pkl",
            "rb",
        )
    )
    config = model.config
    ...
    pd.DataFrame(
        ...
        f"predictions_sycophancy/{dataset_id}_{model_id}_answers_L{chosen_layer}_{scale}_mlp.csv"
    )
