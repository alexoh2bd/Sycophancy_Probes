from probe.utils import load_model, load_test_data
import pickle
import torch
import pyvene as pv
import argparse
...
import importlib
from probe import utils
from probe.utils import generate_and_decode_new_tokens, to_request


def get_probe_vectors(
    chosen_layer, model_id, scale, num_layers, hidden_dim, use_random_direction=None
):
    ...
    for layer in target_layers:
        current_probe = torch.load(
            f"linear_probe/trained_probe_sycophancy/{model_id}/linear_probe_residual_{layer}.pth"
        )["linear.weight"].squeeze()
        if use_random_direction:
            current_probe = torch.normal(
                mean=current_probe.mean(),
                std=current_probe.std(),
                size=current_probe.shape,
            )
        current_probe = current_probe / torch.norm(current_probe, p=2)
        current_std = torch.load(
            f"linear_probe/trained_probe_sycophancy/{model_id}/std_residual_{layer}.pt"
        )
...
def main():
    ...
    parser.add_argument(
        "--model_id", type=str, default="gemma-3", help="Model ID to use"
    )
    parser.add_argument(
        "--dataset_id", type=str, default="truthfulqa", help="Dataset ID to use"
    )
    parser.add_argument(
        "--chosen_layer",
        type=int,
        default=16,
        help="Number of layer representation to use",
    )
    parser.add_argument(
        "--scale", type=float, default=-5, help="Scale factor for probe vectors"
    )
    parser.add_argument(
        "--use_random_direction",
        action="store_true",
        help="If set, use a random direction instead of a fixed probe vector",
    )
    ...
    accuracies = pickle.load(
        open(
            f"linear_probe/trained_probe_sycophancy/{model_id}/accuracies_dict_residual.pkl",
            "rb",
        )
    )
    config = model.config
    ...
    linear_probes = get_probe_vectors(
        chosen_layer,
        model_id,
        scale=scale,
    ...
    pv_model = (
        pv.IntervenableModel(target_components, model=model) if scale > 0 else model
    )
    ...
    for question in tqdm(questions_test):
        res_1, res_2 = generate_and_decode_new_tokens(
            question, pv_model, processor, model_id
        )
    ...
    pd.DataFrame(
        {
            ...
            "correct_answer": correct_answers_test,
            "initial_answer": initial_answer,
            "final_answer": final_answer,
        }
    ).to_csv(
        f"predictions_sycophancy/{dataset_id}_{model_id}_answers_L{chosen_layer}_{scale}_residual.csv"
    )
