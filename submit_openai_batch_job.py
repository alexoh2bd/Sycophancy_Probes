import pandas as pd
from probe.utils import to_request
import argparse
import json
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=",mha",
        help="Activation type (residual, mlp, mha)",
    )
    parser.add_argument(
        "--chosen_layer", type=int, default=4, help="Which layer to intervene (int)"
    )
    parser.add_argument(
        "--scale", type=float, default=-10, help="Scale factor for probe vectors"
    )
    parser.add_argument(
        "--concept",
        type=str,
        default="sycophancy",
        choices=["sycophancy", "truthful", "random"],
    ...
    requests = [
        to_request(f"{req_desc}-{i}", q, a, p)
        for i, (q, a, p) in enumerate(
            zip(questions_test, correct_answers_test, initial_answer)
        )
    ]
    ...
    batch_input_file = client.files.create(
        file=open(initial_answer_path, "rb"), purpose="batch"
    )
    ...
    requests = [
        to_request(f"{req_desc}-{i}", q, a, p)
        for i, (q, a, p) in enumerate(
            zip(questions_test, correct_answers_test, final_answer)
        )
    ]
    ...
    with open(final_answer_path, "w") as f:
        for item in requests:
            ...
            f.write(json_line + "\n")
    batch_input_file = client.files.create(
        file=open(final_answer_path, "rb"), purpose="batch"
    )
    batch_input_file_id = batch_input_file.id
    client.batches.create(
        input_file_id=batch_input_file_id,
